  ...
  <div class="vue-tree">
    <div class="dom-container" ref="treeRef">
      ...
      <div ref="valueRef" class="value">
        <div class="">
          <!--  <v-img v-if="item.imageUrl"  max-height="50"  max-width="100" contain :src="item.imageUrl"></v-img>-->
          <div class="d-flex justify-center pa-5 text-capitalize">
            <span>
              {{ item.title }}
            </span>
          </div>
        ...
        <v-menu elevation="0" offset-y left>
          ...
          >
          <v-list>
            <v-list-item @click="addChild">
              <v-list-item-title class="text-capitalize"
                >new chlid</v-list-item-title
              >
            </v-list-item>
            <v-list-item v-if="lavel !== 0">
              <v-list-item-title
    ...
    <AddTreeNodeView
      :parent="item"
      v-if="addChildDailog"
      @close="addChildDailog = false"
      :isOpened="addChildDailog"
      @added="added"
    />
  </div>
</template>
...
import nodeModule from "@/store/nodeModule";
...
export default class TreeNode extends Vue {
  ...
  offsetTop = 0;
  imge =
    "https://i.pinimg.com/originals/5c/4d/1a/5c4d1ac934848c57de3b11effb65853a.png";
  first = false;
  mounted() {
    this.first = true;
    this.offsetLeft =
  ...
  deleteChild(child: INode) {
    nodeModule
      .deleteNode({ id: child.id, parentId: child.parent.id })
      .then((x) => {
  ...
  drowLink() {
    if (
      ...
      this.linksContainer.innerHTML +=
        '<path  d=" ' +
        path.toString() +
        '" stroke="#59999b" stroke-width="2" fill="none" />';
    }
  }
  get heigth() {
