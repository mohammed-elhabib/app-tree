<template>
  <v-dialog v-model="dialog" hide-overlay width="50%">
    <v-container>
      <v-row>
      ...
      <v-row>
        <v-col class="d-flex justify-end">
          ...
          <v-btn color="red" class="mx-2" @click="$emit('close')" outlined
            >cancel</v-btn
          >
        </v-col>
      </v-row>
...
export default class AddTreeNodeView extends Vue {
  ...
  mounted() {
    ...
    this.newNode.parent = {id:this.parent.id} as INode;
  }
  add() {
    nodeModule.createNode(this.newNode).then((x) => {
      if (!this.parent.children) this.parent.children = [];
      (this.parent.children as any[]).push(x);
      this.$emit("added");
    });
  }
}
